...
n=length(x)
k=n-floor(n/3)*3
if(k==0) n.gp=c(n,n,n)/3
if(k==1) {l=(n-1)/3; n.gp=c(l,l+1,l)}
if(k==2) {l=(n-2)/3; n.gp=c(l+1,l,l+1)}
left=1:n.gp[1]; center=(n.gp[1]+1):(n.gp[1]+n.gp[2]); right=(n.gp[1]+n.gp[2]+1):n
sort.x=sort(x,index.return=TRUE)
sort.y=y[sort.x$ix]
x.med=c(median(sort.x$x[left]),median(sort.x$x[center]),median(sort.x$x[right]))
y.med=c(median(sort.y[left]),median(sort.y[center]),median(sort.y[right]))
